...
selected_gene_file = "autosome_sex_diff.csv"
...
for sample in samples: 
    ...
    # load selected genes: genes that are assigned to be up in Male or Female
    ...
    for xin in select_data:
        ...
        if (status == "Female"):
            female_genes.append(name)
        elif (status == "Male"):
            male_genes.append(name)

    selected_genes = {}
    selected_genes["female"] = female_genes
    selected_genes["male"] = male_genes


    # collect gene list and indices needed to read and filter annotation gtf files
    all_selected_genes = selected_genes["female"] + selected_genes["male"] 
    ...
    # for genes and promoters, get distributions of female, male, and variable_female for violin plot and print to file

    if (outputDataFile):
        dataoutfile = meth_directory + "autosome_sexdiff_data_" + sample + ".txt"
        datahandle = open(dataoutfile,"w")
        print ("\t".join(["gene","sex","gene_avg_meth","promoter_avg_meth"]), file= datahandle)

    female_genes_meth = []
    male_genes_meth = []
    female_promoters_meth = []
    male_promoters_meth = []

    for g in avg_meth:
        if (g in selected_genes["female"] and g in avg_meth):
            (gene_avg_meth,promoter_avg_meth) = avg_meth[g]
            female_genes_meth.append(gene_avg_meth)
            female_promoters_meth.append(promoter_avg_meth)
            if (outputDataFile):
                print ("\t".join([g,"female",str(gene_avg_meth),str(promoter_avg_meth)]), file= datahandle)
        elif (g in selected_genes["male"] and g in avg_meth):
            (gene_avg_meth,promoter_avg_meth) = avg_meth[g]
            inactive_genes_meth.append(gene_avg_meth)
            inactive_promoters_meth.append(promoter_avg_meth)
            if (outputDataFile):
                print ("\t".join([g,"male",str(gene_avg_meth),str(promoter_avg_meth)]), file= datahandle)
    ...
    if (plotData):
        gene_meth_data = list() 
        gene_meth_data.append(female_genes_meth)
        gene_meth_data.append(male_genes_meth)

        plt.figure(figsize = (6,3))
        index_list = [*range(1, len(selected_genes)+1, 1)] #len(selected_genes) = num categories of selected genes = 3 = female, male, variable
        fig, ax = plt.subplots(nrows=1, ncols=2, sharey = 'row')

        ax[0].set(title="Genes: " + sample + "(" + sample_sex + ")")
        ax[0].violinplot(gene_meth_data,
                    showmeans=False,
                    showmedians=False,
                    showextrema=False)
        ax[0].set_xticks(index_list)
        ax[0].set_xticklabels(["female (n = 5)","male (n = 8"], rotation = 90)
        ax[0].set_ylabel("Average percent methylated")

        promoter_meth_data = list() 
        promoter_meth_data.append(female_promoters_meth)
        promoter_meth_data.append(inactive_promoters_meth)
        promoter_meth_data.append(variable_promoters_meth)

        ax[1].set(title="Promotors: " + sample + "(" + sample_sex + ")")
        ax[1].violinplot(promoter_meth_data,
                    showmeans=False,
                    showmedians=False,
                    showextrema=False)
        ax[1].set_xticks(index_list)
        ax[1].set_xticklabels(["female (n = 5)","male (n = 8)"], rotation = 90)
        ax[1].set_ylabel("Average percent methylated")

        # report output 
        outputfile = meth_directory + "autosexdiff_genes_" + sample + ".png"
